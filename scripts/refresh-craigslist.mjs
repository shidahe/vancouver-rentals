...
const candidates=[];
const diagnostics={discoveredDetailUrls:urls.size,detailChecks:0,expiredOrRemoved:0,missingRentOrBedrooms:0,outOfScope:0,staleOrOutOfArea:0,accepted:0,errors:0};
for(const url of [...urls].slice(0,120)){
  try{
    ...
    if(/this posting has been deleted by its author|this posting has expired|flagged for removal/i.test(text)){diagnostics.expiredOrRemoved++;continue;}
    ...
    if(!bedrooms||!rent){diagnostics.missingRentOrBedrooms++;continue;}
    if(bedrooms<2||bedrooms>4||rent<3500){diagnostics.outOfScope++;continue;}
    ...
    const postId=craigslistPostId(url)||hash(url);
    ...
    if(!recent||!inArea){diagnostics.staleOrOutOfArea++;continue;}
    ...
    candidates.push(c);diagnostics.accepted++;await write(path.join(EVIDENCE,`craigslist-${postId}.json`),{checkedAt:iso,c,textSample:text.slice(0,7000)});
