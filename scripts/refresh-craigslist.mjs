...
for(const card of scopedCards){
  try{
    ...
    const c={...card,title:evidence.title||card.title,address:evidence.addressPrecision==='exact_civic'?evidence.address:null,approximateLocation:evidence.addressPrecision==='intersection'?evidence.address:card.address,addressPrecision:evidence.addressPrecision||'neighborhood',rent:evidence.rent,bedrooms:evidence.bedrooms,bathrooms:evidence.bathrooms??card.bathrooms,sqft:evidence.sqft??card.sqft,geo:evidence.geo||card.geo,images:evidence.images.length?evidence.images.slice(0,12):card.images,postedOrUpdatedAt:evidence.postedOrUpdatedAt||card.postedOrUpdatedAt,description:description.slice(0,2000),type,ac:/\bair conditioning\b|\bA\/?C\b/i.test(evidence.text)?true:null,detailVerified:true,evidenceKind:'current_search_card_and_exact_detail'};
    if(evidence.addressPrecision==='exact_civic')diagnostics.exactAddresses++;
