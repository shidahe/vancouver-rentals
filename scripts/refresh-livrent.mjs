...
const searchSources=(sources.discovery||[]).filter(x=>x.adapter==='livrent-search');
const diagnostics={searchesConfigured:searchSources.length,searchesChecked:0,searchesBlocked:0,searchCircuitOpen:false,searchesSuppressed:0};
let consecutiveBlocked=0;
for(const s of searchSources){
  if(consecutiveBlocked>=3){diagnostics.searchCircuitOpen=true;diagnostics.searchesSuppressed=searchSources.length-diagnostics.searchesChecked;break;}
  try{
    const r=await page.goto(s.url,{waitUntil:'domcontentloaded',timeout:45000}),status=r?.status()??null;
    diagnostics.searchesChecked++;
    sourceHealth[s.id]={checkedAt:iso,status,ok:!!r&&status<400,finalUrl:page.url()};
    if(!r||status>=400){consecutiveBlocked=[403,429].includes(status)?consecutiveBlocked+1:0;if([403,429].includes(status))diagnostics.searchesBlocked++;continue;}
    consecutiveBlocked=0;
    await page.waitForTimeout(2500);
    for(const u of await page.locator('a').evaluateAll(as=>as.map(a=>a.href).filter(Boolean))){if(/^https:\/\/liv\.rent\/rental-listings\/detail\//i.test(u))urls.add(u.split('#')[0].split('?')[0]);}
  }catch(e){diagnostics.searchesChecked++;consecutiveBlocked=0;sourceHealth[s.id]={checkedAt:iso,ok:false,error:String(e)}}
}
const candidates=[];
for(const url of [...urls].slice(0,120)){try{const r=await page.goto(url,{waitUntil:'domcontentloaded',timeout:45000});if(!r||r.status()>=400)continue;await page.waitForTimeout(1800);const text=await page.locator('body').innerText({timeout:10000});const raws=await page.locator('script[type="application/ld+json"]').evaluateAll(ns=>ns.map(n=>n.textContent||'').slice(0,50));const ld=[];for(const raw of raws){try{ld.push(JSON.parse(raw))}catch{}}const address=addressFrom(ld,text),geo=geoFrom(ld),rented=/marked as rented|\bRented\b|no longer accepting applications/i.test(text),beds=field(text,/\b([1-5](?:\.5)?)\s*Beds?\b/i)||field(text,/\b([1-5](?:\.5)?)\s*Bedrooms?\b/i),baths=field(text,/\b([1-5](?:\.5)?)\s*Baths?\b/i),sqft=field(text,/\b([0-9]{3,4})\s*ft²\b/i)||field(text,/\b([0-9]{3,4})\s*(?:sq\.?\s*ft|sqft)\b/i),rent=money(text),unit=unitFrom(address,text),target=targetGeo(geo)||targetWords.test(text),year=field(text,/Year Completed\s*\n?\s*(20\d{2}|19\d{2})/i),parking=/Parking\s*\n?\s*(?:Included|\$|Yes)/i.test(text),pets=/Pets\s*\n?\s*(?:Allowed|Allowed with restrictions)/i.test(text),ac=/air conditioning|air conditioned|central ac|central a\/c/i.test(text);if(!beds||beds<2)continue;const item={source:'liv.rent',url,address,unit,identityKey:identity(address,unit,url),geo,targetArea:target,rented,active:!rented,rent,beds,baths,sqft,buildingYear:year,parking:parking||null,petFriendly:pets||null,ac:ac||null,checkedAt:iso,publishable:false};candidates.push(item);await write(path.join(EVIDENCE,`livrent-${hash(url)}.json`),{checkedAt:iso,...item,textSample:text.slice(0,7000),jsonLd:ld.slice(0,8)});}catch{}}
await browser.close();
await write(path.join(DATA,'livrent-candidates.json'),{refreshedAt:iso,mode:'candidate-only',sourceHealth,diagnostics,candidates});
