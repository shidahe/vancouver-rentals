...
import { isTargetRentalAreaText, LISTING_SCOPE_VERSION, listingScopeEligible } from './discovery-policy.mjs';
import { isTargetWestsideCoordinate, parseRealtylinkAirConditioning, parseRealtylinkCoordinateValues, parseRealtylinkRoomCount } from './realtylink-parser.mjs';
...
const candidates=[];
for(const url of [...urls].slice(0,180)){try{const r=await page.goto(url,{waitUntil:'domcontentloaded',timeout:30000});if(!r||r.status()>=400)continue;await page.waitForTimeout(500);const t=await page.locator('body').innerText();const beds=parseRealtylinkRoomCount(t,'bedroom')||0;const rent=Number((t.match(/\$\s*([0-9,]+)\s*\/month/i)?.[1]||'').replace(/,/g,''))||null;if(!listingScopeEligible({rent,bedrooms:beds},t))continue;const rawSqft=Number((t.match(/Floor Area\s*([0-9,]+)\s*sqft/i)?.[1]||'').replace(/,/g,''))||null;const sqft=rawSqft>=200&&rawSqft<=15000?rawSqft:null;const baths=parseRealtylinkRoomCount(t,'bathroom');const title=t.match(/(?:Apartment|Townhouse|House) for rent\s+([\s\S]{0,140}?),\s*(?:Kitsilano|Arbutus|Point Grey|Dunbar|Quilchena|Vancouver)/i)?.[1]?.trim()||null;const latitude=await page.locator('meta[itemprop="latitude"]').first().getAttribute('content');const longitude=await page.locator('meta[itemprop="longitude"]').first().getAttribute('content');const geo=parseRealtylinkCoordinateValues(latitude,longitude);const lat=geo?.lat??null,lng=geo?.lng??null;if(!isTargetWestsideCoordinate(geo))continue;const rawAddress=(t.match(/##\s+([^\n]+),\s*(?:Kitsilano|Arbutus|Point Grey|Dunbar|Quilchena|Vancouver West|Vancouver)/i)?.[1]||title||'').trim();if(!isTargetRentalAreaText(rawAddress))continue;const unitMatch=rawAddress.match(/^(?:unit\s+)?([A-Za-z0-9-]+)\s+(?=\d{3,5}\b)/i);const unit=unitMatch?.[1]||null;const unitAddress=cleanAddress(unitMatch?rawAddress.slice(unitMatch[0].length):rawAddress);const parking=/Parking Spaces\s+[1-9]/i.test(t);const ac=parseRealtylinkAirConditioning(t);const balcony=/Exterior Features\s+Balcony|\bbalcony\b/i.test(t);const mls=t.match(/MLS[^\n]*No\.\s*([A-Z0-9]+)/i)?.[1]||null;const rawImages=await page.locator('img,source').evaluateAll(nodes=>nodes.flatMap(node=>[node.currentSrc,node.src,node.getAttribute('data-src'),...(node.getAttribute('srcset')||'').split(',').map(x=>x.trim().split(/\s+/)[0])]));rawImages.push(await page.locator('meta[property="og:image"]').first().getAttribute('content'));const images=verifiedPhotoCandidates(rawImages.filter(x=>/media\.realtylink\.org\/images\/consumersite\/property\//i.test(x||'')));candidates.push({source:'Realtylink MLS',url,mls,address:unitAddress||null,unit,type:url.includes('townhouse~')?'townhouse':url.includes('house~')?'house':'condo',rent,bedrooms:beds,bathrooms:baths,sqft,lat,lng,geo:{lat,lng},images,targetArea:true,active:true,parking,ac,balcony,checkedAt:iso,publishable:false,httpStatus:r.status(),bodyText:t.slice(0,30000)});}catch{}}
...
await browser.close();
await write(path.join(DATA,'realtylink-candidates.json'),{refreshedAt:iso,mode:'candidate-only',scopeVersion:LISTING_SCOPE_VERSION,health,candidates});
console.log(`Realtylink: ${candidates.length} target-area live in-scope 2–4BR candidates.`);
