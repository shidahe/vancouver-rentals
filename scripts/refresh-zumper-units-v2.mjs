...
function targetArea(geo) {
  if (!geo) return false;
  // Vancouver West focus: Kitsilano, Arbutus Ridge, Point Grey, Dunbar, Quilchena and immediate nearby areas.
  // Explicitly excludes Downtown/Fairview to the east and UBC/Wesbrook to the west.
...
// Clean transient v2 auto-publications that escaped the target geography, and normalize surviving Zumper records.
...
for(const url of [...urls].slice(0,120)) {
  try {
    ...
    if(!x||!x.live||x.bedrooms!==2||!x.rent||!targetArea(x.exactGeo)) continue;
...
// Build canonical identity map across ALL sources, not just Zumper. This lets exact same unit from MLS/Zumper merge while preserving distinct units at one address.
const byKey = new Map();
...
// Remove duplicate auto-Zumper records when the same canonical address+unit is already represented by a non-Zumper listing.
