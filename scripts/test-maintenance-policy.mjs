...
const rentalscaThrottled=rentalscaLaneHealth({reachable:8,total:9,candidates:0,diagnostics:{searchLeadCount:7,detailUrls:92,detailBlocked:3,detailCircuitOpen:true,detailSuppressed:89}});
if(rentalscaThrottled.healthy||!/7 scoped search-only leads retained/.test(rentalscaThrottled.detail)||!/89 further detail checks suppressed/.test(rentalscaThrottled.detail)||!source.includes('if(consecutiveBlocked>=3)')||!source.includes('diagnostics.detailSuppressed=requestedDetails.length-diagnostics.detailChecked')||!source.includes("evidenceKind:'search_result_lead_only'")||!source.includes('availabilityVerified:false'))failures.push('Rentals.ca search-only leads or repeated 403/429 suppression can silently regress.');
