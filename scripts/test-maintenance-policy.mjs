...
const rentalscaThrottled=rentalscaLaneHealth({reachable:8,total:9,candidates:0,diagnostics:{detailUrls:92,detailBlocked:3,detailCircuitOpen:true,detailSuppressed:89}});
if(rentalscaThrottled.healthy||!/89 further detail checks suppressed/.test(rentalscaThrottled.detail)||!source.includes('if(consecutiveBlocked>=3)')||!source.includes('diagnostics.detailSuppressed=requestedDetails.length-diagnostics.detailChecked'))failures.push('Rentals.ca repeated 403/429 responses can trigger excessive blocked detail requests or hide suppression from coverage.');
