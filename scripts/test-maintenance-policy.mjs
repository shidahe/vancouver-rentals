...
if(!priorityInventoryEvidenceHealthy({ok:true,status:200,checkedAt:'2026-09-12T23:00:00Z',bodyText:'Viridian 1783 14th Ave W 2 beds $3,395 Check availability',source:{url:'https://www.zumper.com/apartment-buildings/p510456/viridian-fairview-vancouver-bc'}},/viridian|1783 (?:west |w )?14th/i,inventoryNow,12,/zumper\.com\/apartment-buildings\/p510456\/viridian-fairview-vancouver-bc/i)||
   !catalog.discovery.some(x=>x.id==='viridian-zumper'&&/p510456\/viridian-fairview-vancouver-bc/.test(x.url))||
   !coverageSource.includes('viridianInventoryEvidence.find')) failures.push('Viridian exact-building fallback can regress when RentCafe is blocked.');
...
const rentalscaNewLead=classifyRentalsCaSearchLead({address:'1960 Macdonald Street, Vancouver, BC'},[{id:'other',address:'2083 W 33rd Ave, Vancouver, BC',availabilityStatus:'active'}]);
const rentalscaActiveLead=classifyRentalsCaSearchLead({address:'1960 Macdonald Street, Vancouver, BC'},[{id:'macdonald-active',address:'1960 Macdonald St, Vancouver, BC',availabilityStatus:'active'}]);
const rentalscaRemovedLead=classifyRentalsCaSearchLead({address:'1960 Macdonald Street, Vancouver, BC'},[{id:'macdonald-removed',address:'1960 Macdonald St, Vancouver, BC',availabilityStatus:'removed'}]);
if(rentalscaNewLead.leadStatus!=='new_unverified_address'||rentalscaNewLead.matchedListingId!==null||rentalscaActiveLead.leadStatus!=='represented_active_address'||rentalscaActiveLead.matchedListingId!=='macdonald-active'||rentalscaRemovedLead.leadStatus!=='needs_detail_revalidation'||rentalscaRemovedLead.matchedAvailabilityStatus!=='removed')failures.push('Rentals.ca search-only leads are not classified against managed inventory safely.');
...
const discoveredKitsWalk=discoveredStructuredInventories(kitsWalkStructuredFixture,{bathrooms:2,availabilitySignals:['move-in ready']});
if(discoveredKitsWalk.length!==2||!discoveredKitsWalk.some(x=>x.key==='2-bedroom-741-3650'&&x.bathrooms===2&&x.dynamicallyDiscovered&&x.availabilitySignals.includes('move-in ready'))) failures.push('Structured Kits Walk inventory cannot become tracked dynamic floorplans.');
...
const rentalscaThrottled=rentalscaLaneHealth({reachable:8,total:9,candidates:0,diagnostics:{searchLeadCount:7,newSearchLeadCount:4,representedSearchLeadCount:2,revalidationSearchLeadCount:1,detailUrls:92,detailBlocked:3,detailCircuitOpen:true,detailSuppressed:89}});
if(rentalscaThrottled.healthy||!/7 scoped search-only leads retained \(4 new, 2 already active, 1 need revalidation\)/.test(rentalscaThrottled.detail)||!/89 further detail checks suppressed/.test(rentalscaThrottled.detail)||!source.includes('if(consecutiveBlocked>=3)')||!source.includes('diagnostics.detailSuppressed=requestedDetails.length-diagnostics.detailChecked')||!source.includes("evidenceKind:'search_result_lead_only'")||!source.includes('availabilityVerified:false')||!source.includes('classifyRentalsCaSearchLead'))failures.push('Rentals.ca search-only leads, inventory classification or repeated 403/429 suppression can silently regress.');
...
if(!source.includes('parseCraigslistSearchCard(raw,raw.baseUrl)')||!source.includes('craigslistDetailEvidence({...detail')||!source.includes("evidenceKind:'current_search_card_and_exact_detail'")||!source.includes("address:evidence.addressPrecision==='exact_civic'?evidence.address:null")||!source.includes('searchCards:rawCards.length')||!source.includes('detailIdentityMismatch:0')||!source.includes('normalizeCraigslistDetailUrl(u,page.url())')||!source.includes('extractCraigslistDetailUrls(await page.content()')||!source.includes('resultContainerCount:resultNodes.length')||!source.includes('hrefShapeSamples:samples')||!source.includes('discoveredDetailUrls:urls.size')||!source.includes('embeddedDetailLinkCount=embeddedLinks.length')) failures.push('Craigslist search-card recovery, exact-detail validation or zero-yield diagnostics can silently regress.');
...
if (!source.includes('rolledBackMls.has(x.mls)') || !source.includes('previousReconciliation.realtylinkSnapshotScope===realtylinkScope&&!rolledBackMls.has(x.mls)')) failures.push('MLS missing evidence can survive a scope change or immediately re-remove a rolled-back listing.');
