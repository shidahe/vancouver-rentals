...
function usable(c){return !!c&&c.active!==false&&!c.rented&&listingScopeEligible(c,c.description||c.bodyText||'')&&c.targetArea!==false&&Number(c.rent)<=15000&&!!c.address&&(!/^craigslist$/i.test(c.source)||c.detailVerified===true&&c.addressPrecision==='exact_civic');}
...
for(const x of payload.listings){
  ...
  if(!/realtylink\.org/i.test(evidence?.sourceUrl||evidence?.finalUrl||'')||!evidence?.ok||Number(evidence.status)<200||Number(evidence.status)>=400||!evidence.identityMatch||
    ...
    !listingScopeEligible({...x,rent,bedrooms:beds},evidence.bodyText||'')||rent>15000)continue;
...
for(const [k,items] of groups){
  ...
  if(!listing&&exactUnit&&families.length>=2&&rentSpread<=300&&items.every(c=>c.publishable!==false)){
...
const mlsDeduped=dedupeMlsRecords([...uniqueListings.values()],history,imageSources);
payload.listings=mlsDeduped.listings;
state.mlsDuplicatesMerged=mlsDeduped.merged;
